...
describe("Comprehensive Counter E2E Tests on Base Sepolia", async function () {
  ...
  it("Should create bypass entry when score is insufficient", async function () {
    ...
    if (attestedData.score >= 100n) {
      console.log(`⚠ Skipping bypass test - TEE score (${attestedData.score}) is already sufficient (required: 100)`);
      this.skip();
      return;
    }
  ...
  it("Should increment bypass entry nonce on subsequent attempts", async function () {
    ...
    if (BigInt(attestation.attestedData.score) >= 100n) {
      console.log(`⚠ Skipping bypass nonce test - TEE score (${attestation.attestedData.score}) is already sufficient`);
      this.skip();
      return;
    }
  ...
  it("Should calculate bypass expiry correctly for different score differences", async function () {
    ...
    // Skip test if score is already sufficient
    if (attestedData1.score >= 100n) {
      console.log(`⚠ Skipping bypass expiry calculation test - TEE score (${attestedData1.score}) is already sufficient`);
      this.skip();
      return;
    }
